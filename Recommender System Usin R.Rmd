...
g1000 <- left_join(g500, movie_data %>% select(id, weighted_rating), by = 'id')
g1000


recommend_fuckinmovies <- function(Genre, Language = 'en'){
  g1000 %>% filter(original_language == Language & genres== Genre) %>%
    arrange(desc(weighted_rating)) %>% slice(1:10)
  
  
    }

movie_data %>% filter(genre_1 == 'Drama') %>%
    arrange(desc(weighted_rating)) %>% slice(1:10)
